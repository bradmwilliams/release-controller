...
func (c *ReleaseMirrorJobStatusController) lookupReleasePayload(obj interface{}) {
...
func (c *ReleaseMirrorJobStatusController) sync(ctx context.Context, key string) error {
	...
	// If the release mirror job status is terminal (Success), then we have noting else to do
	if originalReleasePayload.Status.ReleaseMirrorJobResult.Status == v1alpha1.ReleaseMirrorJobSuccess {
		return nil
	}
