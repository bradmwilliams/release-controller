	...
	for name, verifyType := range release.Config.Verify {
		...
		case verifyType.ProwJob != nil:
			...
			if verifyType.Upgrade {
				upgradeSources, err := c.getUpgradeSources(release, releaseTag, name, verifyType.UpgradeFrom, false)
				if err != nil || len(upgradeSources) == 0 {
					return nil, err
				}
				previousTag = upgradeSources[0].tag
				previousReleasePullSpec = upgradeSources[0].pullSpec
			}
			jobName := name
			if jobRetries > 0 {
...
func (c *Controller) getUpgradeSources(release *Release, releaseTag *imagev1.TagReference, verifyName, upgradeFrom string, periodic bool) ([]upgradeSource, error) {
	...
	case releaseUpgradeFromPreviousMinus1:
		if tags := sortedReleaseTags(release, releasePhaseAccepted); len(tags) > 1 {
			return []upgradeSource{{
				tag:      tags[1].Name,
				pullSpec: release.Target.Status.PublicDockerImageRepository + ":" + tags[1].Name,
			}}, nil
		}
		...
		if status, ok := testStatus[name]; ok {
			for _, jobStatus := range status.Status {
				...
				case releaseVerificationStatePending:
					...
					// we need to process this
				default:
					klog.V(2).Infof("Unrecognized verification status %q for type %s on release %s", jobStatus.State, name, releaseTag.Name)
				}
				break
			}
			// find the next time, if ok run.
...
func (c *Controller) expandCandidateTests(release *Release, releaseTag *imagev1.TagReference) (map[string]*ReleaseCandidateTest, error) {
	...
	for name, testType := range release.Config.CandidateTests {
		if testType.Disabled {
			klog.V(2).Infof("Release testing step %s is disabled, ignoring", name)
			continue
		}
		...
		case testType.ProwJob != nil:
			...
			// if this is an upgrade job, find the appropriate source for the upgrade job
			upgradeSources, err := c.getUpgradeSources(release, releaseTag, name, testType.UpgradeFrom, false)
			if err != nil || len(upgradeSources) == 0 {
				return nil, err
			}

			for _, src := range upgradeSources {
				srcName := name
